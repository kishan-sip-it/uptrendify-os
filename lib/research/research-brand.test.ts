...
import { researchBrand } from './research-brand';
...
function htmlPage(title: string) {
  return `<!DOCTYPE html><html><head><title>${title}</title><meta name="description" content="desc"/></head><body><h1>Hello</h1><p>Brand content here.</p><a href="/about">About</a></body></html>`;
}
...
describe('researchBrand', () => {
  ...
  it('crawls pages and completes with COMPLETED status', async () => {
    ...
    const result = await researchBrand(ORG_ID, BRAND_ID, 'http://8.8.8.8', RUN_ID);
    ...
    const mocked = createSupabaseServerClient as ReturnType<typeof vi.fn>;
    const updates = mocked.mock.results[0].value.__updates;
  ...
  it('returns FAILED when no pages can be processed', async () => {
    vi.stubGlobal('fetch', vi.fn(async () =>
      new Response('not found', { status: 404, headers: { 'content-type': 'text/html' } })
    ));
  ...
  it('rejects private target before fetching', async () => {
    ...
    await expect(researchBrand(ORG_ID, BRAND_ID, 'http://127.0.0.1', RUN_ID)).rejects.toThrow();
