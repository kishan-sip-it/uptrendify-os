...
Implemented for the strategy engine: `strategy_generation` uses the same provider abstraction and transient-retry helper. Input is a capped Brand Brain snapshot (facts, insights, evidence claims, sources) rendered as markdown; output must satisfy a strict Zod schema (`lib/strategy/schema.ts`) covering 14 sections (executive summary, business understanding, 3–5 strategic objectives, ICP, positioning, messaging, content strategy, SEO strategy incl. search intent categories, channel strategy, campaign opportunities, a 90-day roadmap, KPI groups, risks/gaps and assumptions). If the provider returns invalid JSON or fails schema validation a single repair attempt is made; invalid output is never persisted as success — the strategy row is marked `FAILED` with `STRATEGY_VALIDATION_FAILED` or `AI_GENERATION_FAILED`. Strategies are versioned per brand (`(brand_id, version)` unique), job rows in `strategies` carry `status/error_code/error_message/started_at/finished_at`, and an `ai_tasks.strategy_id` link joins to the audit trail (usage, latency, metadata). The Brand Brain must exist first; strategies without brain context fail fast with `INSUFFICIENT_BRAIN` and no AI call is made.
