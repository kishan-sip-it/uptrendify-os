...
export default function SwarmCursor({
  ...
  useEffect(() => {
    ...
    if (reduceMotion) {
      container.dataset.disabled = 'reduced-motion';
      return () => {
    ...
    try {
      ...
      container.dataset.disabled = 'webgl-unavailable';
      return () => {
        delete container.dataset.disabled;
      };
    }
    ...
    const compProgram = new Program(gl, {
      ...
      uniforms: {
        tField: { value: null },
        uColor: { value: hexToRgb(propsRef.current.color ?? '#ffffff') },
        uAccent: { value: hexToRgb(propsRef.current.accentColor ?? '#ffffff') },
        uMerge: { value: propsRef.current.merge ?? 0.77 },
        uGlow: { value: propsRef.current.glow ?? 0.75 },
        uOpacity: { value: propsRef.current.opacity ?? 1 },
      },
      transparent: true,
      depthTest: false,
      depthWrite: false,
    ...
    const onDown = (event: PointerEvent) => {
      ...
      const escape = 620 + (props.speed ?? 2.5) * 130;
      ...
      for (let i = 0; i < MAX; i += 1) {
        ...
        vx[i] = (dx / distance) * kick;
        vy[i] = (dy / distance) * kick;
      }
    ...
    const frame = (now: number) => {
      ...
      for (let i = activeCount; i < particleCount; i += 1) {
        spawn(i, anchorX, anchorY);
      }
      ...
      const particleSpeed = 110 + Math.max(0.1, props.speed ?? 2.5) * 165;
      ...
      const flowMix = (props.wander ?? 0.25) * 2.4;
      const eps = 0.08;
      ...
      for (let i = 0; i < particleCount; i += 1) {
        ...
        const swirl =
          Math.sqrt(Math.max(0, 1 - radial * radial)) * handed[i];
        ...
        const wishLength = Math.hypot(wishX, wishY) || 1e-4;
        wishX /= wishLength;
        wishY /= wishLength;

        const rate = steerRate * agility[i] * (1 - burst);
        let ax = (wishX * particleSpeed - vx[i]) * rate;
        let ay = (wishY * particleSpeed - vy[i]) * rate;

        if (burst > 0.001) {
          ax -= ux * particleSpeed * burst * 5.5;
          ay -= uy * particleSpeed * burst * 5.5;
        }
        ...
        for (let j = 0; j < particleCount; j += 1) {
          ...
          const sx = px[i] - px[j];
          const sy = py[i] - py[j];
          const distanceSquared = sx * sx + sy * sy;

          if (distanceSquared > 1e-4 && distanceSquared < sepDist * sepDist) {
            const separationDistance = Math.sqrt(distanceSquared);
            const force =
              (1 - separationDistance / sepDist) *
              particleSpeed *
              3.2 *
              (props.separation ?? 0.15);
            ax += (sx / separationDistance) * force;
            ay += (sy / separationDistance) * force;
          }
        }

        const accelerationLength = Math.hypot(ax, ay);
        const accelerationCap = maxForce * (1 + burst * 4);

        if (accelerationLength > accelerationCap) {
          ax = (ax / accelerationLength) * accelerationCap;
          ay = (ay / accelerationLength) * accelerationCap;
        }

        vx[i] += ax * dt;
        vy[i] += ay * dt;

        const currentSpeed = Math.hypot(vx[i], vy[i]);
        const highSpeed = particleSpeed * (1 + burst * 3.5);
        const lowSpeed = particleSpeed * 0.32;

        if (currentSpeed > highSpeed) {
          vx[i] = (vx[i] / currentSpeed) * highSpeed;
          vy[i] = (vy[i] / currentSpeed) * highSpeed;
        } else if (currentSpeed < lowSpeed && currentSpeed > 1e-4) {
          vx[i] = (vx[i] / currentSpeed) * lowSpeed;
      ...
      if (lastSample < 0 || nowSec - lastSample >= 0.008) {
        ...
        const historyBase = histHead * MAX;

        for (let i = 0; i < particleCount; i += 1) {
          histX[historyBase + i] = px[i];
          histY[historyBase + i] = py[i];
        }

        histHead = (histHead + 1) % HISTORY;
        if (histLen < HISTORY) histLen += 1;
      }

      const trailAge = (props.trail ?? 0.75) * 0.85;
      const perAgent = Math.max(0, Math.floor(MAX_QUADS / particleCount) - 1);
      const maxStamps = Math.min(46, perAgent);

      let quad = 0;

      const pushQuad = (cx: number, cy: number, radius: number, weight: number) => {
        const v = quad * 8;
        positions[v] = cx - radius;
        positions[v + 1] = cy - radius;
        positions[v + 2] = cx + radius;
        positions[v + 3] = cy - radius;
        positions[v + 4] = cx + radius;
        positions[v + 5] = cy + radius;
        positions[v + 6] = cx - radius;
        positions[v + 7] = cy + radius;

        const weightOffset = quad * 4;
        weights[weightOffset] = weight;
        weights[weightOffset + 1] = weight;
        weights[weightOffset + 2] = weight;
        weights[weightOffset + 3] = weight;
        quad += 1;
      };

      for (let i = 0; i < particleCount; i += 1) {
        const headRadius = (props.size ?? 10) * scale[i] * 2.1;
        const headWeight = 1.06 + 0.3 * scale[i];
        pushQuad(px[i], py[i], headRadius, headWeight);

        if (trailAge < 0.01 || maxStamps < 2 || histLen < 2) continue;

        const step = Math.max(2, (props.size ?? 10) * scale[i] * 0.5);
        ...
        for (let j = 0; j < histLen && stamps < maxStamps; j += 1) {
          const slot = (histHead - 1 - j + HISTORY) % HISTORY;
          if (nowSec - histT[slot] > trailAge) break;

          const historyX = histX[slot * MAX + i];
          const historyY = histY[slot * MAX + i];
          const segmentX = historyX - prevX;
          const segmentY = historyY - prevY;
          const segmentLength = Math.hypot(segmentX, segmentY);

          if (segmentLength < 1e-4) continue;

          while (nextAt <= walked + segmentLength && stamps < maxStamps) {
            const f = (nextAt - walked) / segmentLength;
            const u = nextAt / span;
            const taper = Math.pow(Math.max(0, 1 - u), 0.55);
            const radius = headRadius * taper;
            ...
            const stampWeight = Math.min(
              headWeight,
              (headWeight * step) / (radius * 0.934),
            );
      ...
      compProgram.uniforms.uColor.value = hexToRgb(props.color ?? '#ffffff');
      compProgram.uniforms.uAccent.value = hexToRgb(props.accentColor ?? '#ffffff');
      compProgram.uniforms.uMerge.value = props.merge ?? 0.77;
      compProgram.uniforms.uGlow.value = props.glow ?? 0.75;
      compProgram.uniforms.uOpacity.value = props.opacity ?? 1;
