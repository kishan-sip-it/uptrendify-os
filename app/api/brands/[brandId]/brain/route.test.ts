  ...
  it('returns approved facts, insights, suggestions, sources and import info', async () => {
    const client = makeClient([
      ...
      [
        ...
        {
          data: [
            { id: 's1', field: 'brand_name', label: 'Brand name', kind: 'text', proposed_value: 'Aurora', status: 'PENDING', evidence: [], evidence_strength: 'weak', sources_examined: 1, confidence: 0.6, created_at: 'x', updated_at: 'x', reviewed_at: null },
            { id: 's2', field: 'buyer_personas', label: 'Ideal customer profiles', kind: 'persona', proposed_value: null, status: 'NOT_FOUND', evidence: [], evidence_strength: null, sources_examined: 1, confidence: 0.6, created_at: 'x', updated_at: 'x', reviewed_at: null },
          ],
          error: null,
        },
      ],
      [
        'research_sources',
        {
          data: [{ status: 'PROCESSED' }, { status: 'PROCESSED' }, { status: 'FAILED', error_message: 'HTTP 404' }],
          error: null,
        },
      ],
    ]);
    ...
    expect(body.facts[0].key).toBe('brand_name');
    expect(body.insights).toHaveLength(1);
    expect(body.sources).toHaveLength(1);
    expect(body.suggestions).toHaveLength(2);
    expect(body.suggestionCounts).toMatchObject({ pending: 1, notFound: 1, needsReview: 1 });
    expect(body.importInfo).toMatchObject({ processed: 2, failed: 1 });
    expect(body.latestRun).toMatchObject({ id: RUN_ID, status: 'COMPLETED', ai: { provider: 'gemini' } });
  });

  it('excludes unapproved facts from the authoritative facts list', async () => {
    const client = makeClient([
      ['brands', { data: { id: BRAND_ID }, error: null }],
      ...
      ['research_sources', { data: [], error: null }],
    ]);
    mocks.createSupabaseServerClient.mockResolvedValue(client);
