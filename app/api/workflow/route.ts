...
export async function GET() {
  try {
    ...
    const brandsResult = await supabase.from('brands')
    ...
    if (brands.length === 0) {
      return NextResponse.json({
        ...
        nextHref: '/brands/new',
        message: 'Add a brand so the system can research it.',
      });
    }
    ...
    if (researchActive) {
      ...
      currentKey = 'campaigns';
      nextAction = 'Review campaign performance and add content';
    }
    ...
    const nextHref = currentKey === 'research'
      ...
      : currentKey === 'brand_brain'
        ? '/brands/' + currentBrandId + '#intelligence'
        : currentKey === 'strategy'
          ? '/brands/' + currentBrandId + '#strategy'
          : currentKey === 'content'
            ? '/content'
            : currentKey === 'campaigns'
              ? '/campaigns'
    ...
    return NextResponse.json({
      ...
      blocker,
      message: null,
      progressPercent: Math.round((Math.max(0, index) / STAGES.length) * 100),
    });
  } catch (error) {
